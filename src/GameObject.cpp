...
class $modify(MyGameObject, GameObject) {
	...
	void activateObject() {
		if (!Utils::modEnabled() || LevelEditorLayer::get()) return this->GameObject::activateObject();
		...
		if (noEffects == "Enable on Visible, Disable on Invis.") {
			this->m_hasNoEffects = !this->m_isHide && !this->m_isInvisible && this->GameObject::getOpacity() > 0;
			...
			bool isInvis = this->m_isHide || this->m_isInvisible || this->GameObject::getOpacity() <= 0;
